...
// Funções para persistir a sessão do WhatsApp no Supabase
const sessionStore = {
    async restore(sessionId) {
        ...
        if (error && error.code !== 'PGRST116') { // PGRST116 é "não encontrado"
            console.error('Erro ao carregar sessão do Supabase:', error.message);
            return null;
        }
...
client.on('authenticated', (session) => {
    // A autenticação agora é gerenciada pelo LocalAuth
...
setInterval(async () => {
    try {
        ...
        for (const cliente of clientes) {
            ...
            // Atualiza o status de follow-up no Supabase
            await supabase
                .from('clientes_atendidos')
                .update({ followup_enviado: true })
                .eq('numero', cliente.numero);
        }
    } catch (err) {
